...
class PriceGenerator:
    ...
    def get_smart_gas_limit(self, tokens_count: int, attempt: int = 0) -> int:
        """Умная оценка газа на основе реальных измерений"""

        # Оптимизированные базовые значения (на основе успешного результата 4.93M для 6 токенов)
        if tokens_count == 1:
            base_gas = 800000  # Увеличено с 500k
        elif tokens_count <= 3:
            base_gas = 2500000  # Для малых batch
        else:
            # Для 6 токенов нужно ~4.9M, добавляем 10% буфер
            base_gas = 1500000 + (tokens_count * 700000)  # ~5.7M для 6 токенов

        # Увеличение при повторных попытках (если всё-таки не хватило)
        retry_multiplier = 1.0 + (attempt * 0.2)  # +20% за попытку

        # Увеличение при последовательных неудачах
        failure_multiplier = 1.0 + (self.failed_attempts * 0.1)  # +10% за неудачу
    ...
    def _load_config(self):
        ...
        self.contract_addresses = config['contracts']
        ...
        for symbol, token_data in config['tokens'].items():
            initial_price = float(config['initialPrices'].get(symbol, "1.0"))
    ...
    async def initialize(self, rpc_url: str = "http://localhost:8545", private_key: str = None):
        ...
        if private_key:
            ...
            self.keeper_account = Account.from_key(list(hardhat_keys.values())[0])
        ...
        self.router = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.contract_addresses['Router']),
            abi=router_abi
        )

        self.access_control = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.contract_addresses['AccessControl']),
            abi=access_abi
        )

        # Детальная проверка прав и контрактов
        try:
            # Проверить что keeper имеет необходимую роль
            ...
            # УБРАТЬ проверку ROUTER_ROLE - её может не быть
            # Вместо этого проверим доступные функции в AccessControl
            self.logger.info("✅ Available functions in AccessControl:")
            for func in self.access_control.all_functions():
                if 'ROLE' in func.function_identifier:
                    self.logger.info(f"  - {func.function_identifier}")

            # Проверить что Oracle адрес корректный и доступен
            ...
            # Проверить что Oracle принимает обновления от Router
            # Это можно сделать попробовав call на одну из функций
            self.logger.info("✅ Oracle contract accessible")

        except Exception as e:
            self.logger.error(f"Permission check failed: {e}")
            # Не прерывать выполнение, просто логировать
    ...
    async def update_prices_with_retry(self, tokens: List[str], prices: List[int], updates: Dict[str, float]) -> bool:
        ...
        for attempt in range(self.price_config.retry_attempts):
            try:
                ...
                # ДОБАВИТЬ: Проверка метода существует
                try:
                    # Попробовать call для проверки
                    if len(tokens) == 1:
                        self.logger.info(f"Testing updateOraclePrice call for {tokens[0]}")
                        # Это должно показать revert reason если есть
                    ...
                    # Если call не работает, то и transaction не будет работать
                    return False
                ...
                # Проверить специфические типы ошибок
                ...
                # Импортировать traceback для полного стека
                import traceback
...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='../config/deployment-config.json',
...
"""
# С debug режимом
python price_generator_cly_debug.py --debug

# Или с verbose режимом
python price_generator_cly_debug.py --verbose
